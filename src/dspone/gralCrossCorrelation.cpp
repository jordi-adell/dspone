...
*/
#include <dspone/gralCrossCorrelation.h>
...
void GeneralisedCrossCorrelation::allocate()
{
								    ...
								    BaseType *samplesDelay, int numSteps, FFTtype usedFFTtype)
{
  ...
  else if (usedFFTtype == TWOSIDEDFFT)
  {
    if (length != _length)
    {
      throw(DspException("Wrong vector length"));
...
void GeneralisedCrossCorrelation::precomputeTauMatrix(BaseType *samplesDelay, int numSteps, int length, FFTtype usedFFTtype)
{
  ...
  if ( ((usedFFTtype == ONESIDEDFFT) && (length != (_length/2+1))) || ((usedFFTtype == TWOSIDEDFFT) && length != _length) )
  {
    throw(DspException("Wrong vector length"));
  }
...
void GeneralisedCrossCorrelation::generateTwoSidedFFT(const BaseTypeC *x, const BaseTypeC *y, BaseTypeC *fullx, BaseTypeC *fully, int length)
{
  //Generate fullx
...
void GeneralisedCrossCorrelation::calculateCorrelationsCore(const  BaseTypeC *x, const  BaseTypeC *y, BaseTypeC *c,
...
void GeneralisedCrossCorrelation::phatWeight(const  BaseTypeC *x, const  BaseTypeC *y,  BaseTypeC *w)
{
...
void GeneralisedCrossCorrelation::computePhaseRamp(BaseType tau)
{
...
void GeneralisedCrossCorrelation::debugPlot(const BaseTypeC* left, const BaseTypeC *right, const BaseTypeC *c, int length, int numSteps, float tau)
{
